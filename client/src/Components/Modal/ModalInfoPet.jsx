...
function AnimalFoo({ property, values }) {
  ...
  const handleInquireAdoptionClick = () => {
    if (PawrentIsLoggedIn) { 
      history.push('/Questionnaire'); //Add Questionnaire Link Here//
    } else if (ShelterIsLoggedIn) {
      // Add a code here that hides the "Inquire Adoption" button for Shelters//
    } else {
      history.push('/LoginForm'); //Add path to Login Form//
    }
  };
